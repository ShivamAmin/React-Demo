...
const CourseForm = ({course, allAuthors, onSave, onChange, loading, error}) => {
    return (
        <form>
            ...
            <TextInput name="category" label="Category" value={course.category} onChange={onChange} error={errors.authorId}/>
            <TextInput name="length" label="Length" value={course.length} onChange={onChange} error={errors.category}/>
